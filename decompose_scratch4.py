...
# Recover an actual zero-violation instance explicitly and verify it hard.
best_zero = None
rnd7 = _r.Random(9001)
for gi, gfb in enumerate(good_fb):
    for _ in range(6):
        order0 = list(range(1,15))
        rnd7.shuffle(order0)
        v, o = hillclimb(gfb, order0, iters=150)
        if v == 0:
            best_zero = (gfb, o)
            break
    if best_zero:
...
assert best_zero is not None, "no zero found in this rerun"
fbz, oz = best_zero
tz = build(fbz, oz)
table.validate(tz)
bad, inst = total_parity_violations(tz)
print("RECHECK total_parity_violations:", bad, "of", inst)
print("table.count (naive triangles) on 15-line table:", table.count(tz))
print("fb used:", fbz)
print("row15 order:", oz)
print("row lengths:", [len(r) for r in tz])
# sanity: is it simple (no parallels, no concurrences)?
has_bracket = any(isinstance(e,(list,tuple)) for row in tz for e in row)
print("has bracket (concurrence):", has_bracket)
print("any row length != 14:", [len(r) for r in tz if len(r)!=14])
